...
export default function ClientTicketDetail() {
  ...
  const load = () =>
    api.get(`/tickets/${id}`).then((r) => r.ok && setTicket(r.data));
  useEffect(() => {
    load();
  }, [id]);
  ...
  return (
    <div className={styles.page}>
      ...
      <p style={{ color: "#6b7280", marginBottom: 8 }}>
        <span
          className={`${styles.badge} ${STATUS_BADGE[ticket.status] ?? styles.badgeGray}`}
        >
        ...
        {ticket.property && (
          <>
            {" "}
            &middot; {ticket.property.address}, {ticket.property.city}
          </>
        )}
      </p>
      ...
      <div className={styles.messages}>
        {ticket.messages?.map((m: any) => (
          <div
            key={m.id}
            className={`${styles.message} ${["EMPLOYEE", "ADMINISTRATOR"].includes(m.author?.rank) ? styles.messageStaff : ""}`}
          >
            <div className={styles.messageMeta}>
              <strong>{m.author?.name}</strong>
              {["EMPLOYEE", "ADMINISTRATOR"].includes(m.author?.rank) &&
                " (Staff)"}
              {" — "}
